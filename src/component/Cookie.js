...
const Cookie = () => {
  ...
  useEffect(() => {
    const cookieConsent = Cookies.get('Incoso-cookie');
    if (!cookieConsent) {
      setIsVisible(true);
    }
  }, []);
  ...
  const handleAcceptAll = () => {
    Cookies.set('Incoso-cookie', 'Alle akzeptieren', { expires: 1 });
    setIsVisible(false); // Close the popup
  };

  const handleAcceptEssentials = () => {
    Cookies.set('Incoso-cookie', 'Nur essenzielle Cookies akzeptieren', { expires: 1 });
    setIsVisible(false); // Close the popup
  };
